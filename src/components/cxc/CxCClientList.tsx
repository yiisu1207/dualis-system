...
import { Search, Plus, AlertTriangle, CheckCircle, Clock, ShieldCheck, Wallet, TrendingUp, TrendingDown, Users, Inbox } from 'lucide-react';
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-3 pb-3 space-y-3 border-b border-slate-100 dark:border-white/[0.06]">
        ...
        <div className="flex flex-wrap gap-1.5 -mx-0.5">
          ...
          {filterPill('ZERO', 'Sin deuda', counts.zero, CheckCircle, 'bg-emerald-500/15 text-emerald-500 dark:text-emerald-400')}
        </div>
          ...
          <div className="px-4 py-16 flex flex-col items-center text-center">
            <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/[0.05] flex items-center justify-center mb-3">
              {filter === 'OVERDUE' ? <AlertTriangle size={22} className="text-rose-400/60" />
                : filter === 'AT_LIMIT' ? <Clock size={22} className="text-indigo-400/60" />
                : filter === 'ZERO' ? <CheckCircle size={22} className="text-emerald-400/60" />
                : filter === 'PENDING' ? <Inbox size={22} className="text-amber-400/60" />
                : search ? <Search size={22} className="text-slate-300 dark:text-white/20" />
                : <Users size={22} className="text-slate-300 dark:text-white/20" />}
            </div>
