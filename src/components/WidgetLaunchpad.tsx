            ...
            return (
              <button
                ...
                className={`group relative p-8 rounded-[2.5rem] border transition-all duration-500 text-left flex flex-col justify-between h-64 overflow-hidden hover:shadow-2xl ${
                  isActive 
                    ? 'bg-white border-indigo-200 shadow-indigo-100 shadow-xl' 
                    : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 grayscale hover:grayscale-0'
                }`}
              >
                <div className="flex justify-between items-start relative z-10">
                  <div className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-all duration-500 group-hover:scale-110 ${isActive ? w.color : 'bg-slate-100 text-slate-400'} text-white`}>
                    <w.icon size={28} />
                  </div>
                  {isActive && (
                ...
                <div className="relative z-10 mt-6">
                  <h3 className={`text-lg font-black tracking-tight mb-1 ${isActive ? 'text-slate-900' : 'text-slate-400'}`}>
                    {w.name}
                  </h3>
                ...
                <div className="mt-6 flex items-center justify-between relative z-10">
                   ...
                   <div className={`h-8 w-8 rounded-full flex items-center justify-center transition-all ${isActive ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-300'}`}>
                     {isActive ? <LayoutGrid size={14} /> : <Zap size={14} />}
                   </div>
                </div>

                {/* BACKGROUND DECORATION */}
                <div className={`absolute -right-4 -bottom-4 opacity-5 transition-transform duration-700 group-hover:scale-125 group-hover:rotate-12 ${isActive ? 'text-indigo-600' : 'text-slate-900'}`}>
                  <w.icon size={120} />
                </div>
              </button>
            );
