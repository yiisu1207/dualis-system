...
import type { BusinessBankAccount, ReconciliationBatch } from '../../../types';
...
export default function ReceiptBatchModal({
  ...
  // Conflicto de nombre — detectado al pulsar "Procesar"
  ...
  const runProcessing = useCallback(async (existingBatch?: ReconciliationBatch) => {
    setStep(3);
    setProcessing(true);
  ...
  // Paso previo: si ya existe un lote con el mismo nombre, ofrecer Fusionar/Reemplazar.
  ...
  }, [businessId, name, runProcessing]);

  const handleMerge = useCallback(async () => {
    const existing = nameConflict;
    if (!existing) return;
    setNameConflict(null);
    await runProcessing(existing);
  }, [nameConflict, runProcessing]);
  ...
  }, [nameConflict, businessId, runProcessing]);
        ...
        <div className="sticky bottom-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between">
          ...
          {step === 2 && (
            <button
              ...
              disabled={!canProcess || checkingConflict}
              className="px-5 py-2 bg-indigo-600 dark:bg-indigo-500 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-40 inline-flex items-center gap-1.5"
            >
              {checkingConflict && <Loader2 size={12} className="animate-spin" />}
            ...
            <div key={it.id} className="relative bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-xs">
              ...
              {it.kind === 'image' ? (
                ...
                <>
                  ...
                  <div className="text-slate-700 dark:text-slate-200">Bs {it.amount.toFixed(2)}</div>
        ...
        {results.map((r: ProcessingResult, i: number) => (
          <div key={i} className="px-3 py-2 text-xs flex items-center gap-2">
            ...
            <span className="flex-1 truncate text-slate-700 dark:text-slate-200">
              {r.draft.kind === 'image' ? r.draft.file.name : `Manual · Ref ${r.draft.reference} · Bs ${r.draft.amount.toFixed(2)}`}
            </span>
            {r.abono && (
